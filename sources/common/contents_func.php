...
class cutil_ex  {
	...
	*/
	...
	public static function chkset_err_field(&$err_array,$name,$field,$chkid,$para1='',$para2=''){
		switch($chkid){
			...
			case 'isset_pass':
				...
				elseif(!cutil_ex::chek_pass_format($_POST[$name])){
					$err_array[$name] = "「{$field}」が書式にあってません";
					return true;
				}
			break;
	...
	//--------------------------------------------------------------------------------------
	/*!
	@brief	エラーHTMLを返す(CSS依存)
	@param[in]	$key 添え字
	@param[in]	$flg 最初に改行を付けるかどうか
	@return	フォーマットに合っていればtrue
	*/
	//--------------------------------------------------------------------------------------
	public static function err_html($key,$flg = true){
		global $err_array;
		$br = '';
		if($flg){
			$br = '<br>';
		...
		$err_str = '';
		if(isset($err_array[$key])){
			$err_str = <<< END_BLOCK
{$br}<small class="text-danger">{$err_array[$key]}</small>
END_BLOCK;
		}
		return $err_str;
	}
